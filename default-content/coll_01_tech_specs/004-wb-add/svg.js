(function() {
    let cv = d3wb.config()
        .attr('margin', '40 40 50 60')
        .toCanvas();

    // create some data sets
    let data = Array(...Array(1007)).map(function(curr, i) {
        return {
            'x': i,
            'y': Math.round(Math.random() * 1000),
        };
    });
    let dataSmooth = d3wb.util.smoothData(data, 'x', 'y', 10);
    let dataSmoothMore = d3wb.util.smoothData(data, 'x', 'y', 100);
    let sets = [data, dataSmooth, dataSmoothMore];
    let pt = 0;
